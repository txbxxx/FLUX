...
- 列出/更新/删除本地已创建快照
- 查看版本历史并恢复历史版本
- 远端 Git 仓库管理（add/list/remove）
- 同步推送（push）和拉取（pull）含冲突检测
- 通过终端交互界面执行基础操作
...
- `ai-sync get`
- `ai-sync setting`
- `ai-sync setting create`
- `ai-sync setting list`
- `ai-sync setting get`
- `ai-sync setting delete`
- `ai-sync setting switch`
- `ai-sync snapshot create`
- `ai-sync snapshot list`
- `ai-sync snapshot delete`
- `ai-sync snapshot restore`
- `ai-sync tui`
- `ai-sync completion`
...
```powershell
.\ai-sync.exe setting create --name <name> --token <token> --api <api> [--opus-model <model>] [--sonnet-model <model>]
.\ai-sync.exe setting list [--limit <n>] [--offset <n>]
...
```powershell
.\ai-sync.exe setting create --name glm3 --token sk-ant-xxx --api https://api.anthropic.com --opus-model claude-3-opus-20240229 --sonnet-model claude-3-sonnet-20240229
```
...
```powershell
.\ai-sync.exe setting list
.\ai-sync.exe setting list --limit 10 --offset 0
```
...
**单个获取**：
```powershell
.\ai-sync.exe setting get glm3
```

**批量获取**：
```powershell
.\ai-sync.exe setting get glm3 glm5 test_ok
```
...
**单个删除**：
```powershell
.\ai-sync.exe setting delete glm3
```

**批量删除**（带确认）：
```powershell
.\ai-sync.exe setting delete glm3 glm5
```
...
```powershell
.\ai-sync.exe snapshot restore "升级前备份" --dry-run
```
...
6. 配置出问题时，先 `ai-sync snapshot restore <name> --dry-run` 预览变更
7. 确认后执行 `ai-sync snapshot restore <name>` 恢复配置
8. 如果希望交互式浏览，再执行 `ai-sync tui`
