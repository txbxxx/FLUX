...
| 场景 | 命令 | 说明 |
|------|------|------|
| 快照 vs 快照 | `fl snapshot diff <id1> <id2>` | 对比两个历史快照 |
| 快照 vs 文件系统 | `fl snapshot diff <id>` | 对比快照与当前磁盘状态 |
...
```bash
# 摘要模式：快照 vs 快照
fl snapshot diff abc123 def456

# 摘要模式：快照 vs 文件系统
fl snapshot diff abc123

# 内容 diff
fl snapshot diff abc123 def456 -v

# 并排显示
fl snapshot diff abc123 def456 -v --side-by-side

# 按工具过滤
fl snapshot diff abc123 def456 --tool claude

# 按路径过滤
fl snapshot diff abc123 def456 --path "mcp/*"

# 组合使用
fl snapshot diff abc123 --tool claude --path "settings*"

# 恢复预览（复用 diff 输出）
fl snapshot restore abc123 --dry-run
```
