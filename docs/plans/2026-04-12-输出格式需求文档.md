...
```bash
# 快照列表
fl snapshot list --format=json
fl snapshot list --format=yaml
fl snapshot list --format=table    # 默认，可省略

# 扫描结果
fl scan list --format=json
fl scan list --format=yaml
...
# 快照对比
fl snapshot diff abc123 def456 --format=json
fl snapshot diff abc123 def456 --format=yaml

# 组合使用
fl snapshot list --format=json --limit=10 --offset=0 | jq '.items[] | select(.file_count > 10)'
```
