...
添加 `fl setting edit <name>` 命令，支持两种交互模式：
...
```bash
# 基本用法
fl setting edit <name> [flags]

# 示例：只修改 Token
fl setting edit my-config -t new-token

# 示例：修改多个字段
fl setting edit my-config -t new-token -a https://api.new.com
...
fl setting edit <name> -e --format yaml
```
...
```
1. 读取当前配置
2. 生成临时文件路径（/tmp/fl-edit-<name>.<ext>）
3. 根据格式生成配置内容：
   - Token 字段留空 + 注释说明
   - 其他字段预填当前值
4. 调用 $EDITOR 打开临时文件
