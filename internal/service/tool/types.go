...
type ToolInstallation struct {
	...
	ProjectName  string             `json:"project_name,omitempty"`  // 项目名称（仅项目作用域）
	ProjectPath  string             `json:"project_path,omitempty"`  // 项目路径（仅项目作用域）
...
func (r *ToolDetectionResult) ToInstallations() []ToolInstallation {
	...
	for _, project := range r.ProjectInstallations {
		if project != nil && project.Status != StatusNotInstalled {
