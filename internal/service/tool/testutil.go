...
func CreateMockConfigDir(t *testing.T, toolType ToolType, scope ConfigScope) string {
	...
	baseDir := filepath.Join(os.TempDir(), "ai-sync-test", string(toolType), string(scope))
...
func CleanupMockDir(t *testing.T, path string) {
	...
	if path != "" {
		_ = os.RemoveAll(filepath.Join(os.TempDir(), "ai-sync-test"))
	}
}
...
func CreateMockProject(t *testing.T, name string, hasCodex, hasClaude bool) string {
	...
	projectDir := filepath.Join(os.TempDir(), "ai-sync-test-projects", name)
...
func CleanupMockProjects(t *testing.T) {
	t.Helper()
	_ = os.RemoveAll(filepath.Join(os.TempDir(), "ai-sync-test-projects"))
}
